...
@Component({
  selector: "app-collaborators-form-page",
  templateUrl: "./collaborators-form-page.component.html",
  styleUrls: ["./collaborators-form-page.component.scss"],
})
export class CollaboratorsFormPageComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    document.title = "Collaborators Application - First Launch";
    ...
    try {
      if (this.form.valid && this.privacyPolicyAccepted) {
        ...
        await this.joinUploads()
          .pipe(
            switchMap(({ profilePicture, resume }) => {
              ...
              return this.collaboratorsService.registerCollaborator(
                patchedValues
              );
            })
          )
        ...
        await this.router.navigate(["/auth/login"], {
          queryParams: { redirectUrl: "/collaborator" },
        });
      }
    } catch (err) {
  ...
  private joinUploads(): Observable<{
    ...
    if (this.form.get("profilePicture")?.value) {
      ...
      const imageKey = `public/collaborators/pic___${uuid()}___${file.name}`;
    ...
    if (this.form.get("resume")?.value) {
      ...
      const imageKey = `public/collaborators/cv___${uuid()}___${file.name}`;
