...
@Component({
  selector: "app-partnership-form-page",
  templateUrl: "./partnership-form-page.component.html",
  styleUrls: ["./partnership-form-page.component.scss"],
})
export class PartnershipFormPageComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    document.title = "Partnership Application - First Launch";
    ...
    try {
      if (this.form.valid && this.privacyPolicyAccepted) {
        ...
        await this.joinUploads()
          .pipe(
            switchMap(({ profilePicture, resume }) => {
              ...
              return this.partnersService.registerPartner(patchedValues);
            })
          )
          .toPromise();
        ...
        await this.router.navigate(["/auth/login"], {
          queryParams: { redirectUrl: "/partner" },
        });
      }
    } catch (err) {
  ...
  private joinUploads(): Observable<{
    ...
    if (this.form.get("profilePicture")?.value) {
      ...
      const imageKey = `public/partners/pic___${uuid()}___${file.name}`;
    ...
    if (this.form.get("resume")?.value) {
      ...
      const imageKey = `public/partners/cv___${uuid()}___${file.name}`;
